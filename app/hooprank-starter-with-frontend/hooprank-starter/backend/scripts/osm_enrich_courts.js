/**
 * OSM Court Data Enrichment (Optimized)
 * 
 * Re-queries OSM and only updates courts that have richer data available.
 * Only ~3% of OSM basketball courts have additional tags like name, addr:city,
 * operator, or description, so this script is much faster than a full re-import.
 */
...
function overpassQuery(query, retries = 3) {
    return new Promise((resolve, reject) => {
        ...
        const req = https.request({
            ...
            res.on('data', chunk => data += chunk);
            res.on('end', async () => {
                if (res.statusCode === 504 || res.statusCode === 429 || data.includes('runtime error')) {
                    if (retries > 0) {
                        console.log(`    Rate limited (${res.statusCode}), retrying in 30s...`);
                        await sleep(30000);
                        try { resolve(await overpassQuery(query, retries - 1)); }
                        catch (e) { reject(e); }
                        return;
                    }
                    reject(new Error(`API error ${res.statusCode} after retries`));
                    return;
                }
...
// Must match original import script
...
async function main() {
    console.log('╔══════════════════════════════════════════════════════╗');
    console.log('║  OSM COURT DATA ENRICHMENT (Optimized)               ║');
    console.log('║  Only updates courts with richer OSM tags (~3%)      ║');
    console.log('╚══════════════════════════════════════════════════════╝\n');

    let totalUpdated = 0;
    let totalWithRicherData = 0;
    let regionNum = 0;

    for (const region of REGIONS) {
        regionNum++;
        const stateAbbr = STATE_ABBR[region.name] || region.name;

        // Query OSM for ALL tags
...
out center tags;`;
        ...
        // Filter to only elements that have enrichable data
        const enrichable = elements.filter(el => {
            const t = el.tags || {};
            return t.name || t.operator || t.description || t['addr:city'] ||
                t['addr:street'] || t['addr:suburb'] || t['addr:town'];
        });

        console.log(`  [${regionNum}/${REGIONS.length}] ${region.name}: ${elements.length} total, ${enrichable.length} enrichable`);

        let regionUpdated = 0;
        for (const el of enrichable) {
            ...
            // Build richer name
            let name = 'Basketball Court';
            if (tags.name && tags.name.toLowerCase() !== 'basketball court') {
                name = tags.name;
            } else if (tags.operator) {
                name = `${tags.operator} Courts`;
            } else if (tags.description) {
                // Capitalize first letter
                name = tags.description.charAt(0).toUpperCase() + tags.description.slice(1);
            } else if (tags['addr:street']) {
                const num = tags['addr:housenumber'] ? tags['addr:housenumber'] + ' ' : '';
                name = `Basketball Court at ${num}${tags['addr:street']}`;
            }

            // Build richer city  
            const city = tags['addr:city'] || tags['addr:suburb'] || tags['addr:town'] ||
                tags['addr:hamlet'] || tags['addr:municipality'] || null;
            const cityField = city ? `${city}, ${stateAbbr}` : `${region.name}, ${stateAbbr !== region.name ? stateAbbr : 'US'}`;
            ...
            // The original import used the tags-based name as well — we need to match the ID
            // Original: c.name || 'Basketball Court' where name = el.tags?.name || el.tags?.description || null
            const originalOsmName = tags.name || tags.description || null;
            const originalImportName = originalOsmName || 'Basketball Court';
            const id = makeId(originalImportName, roundedLat, roundedLng);
            ...
            const params = new URLSearchParams({
                ...
                lng: String(roundedLng),
                indoor: 'false',
                rims: tags.hoops || '2',
                access: tags.access === 'private' ? 'private' : 'public',
                source: 'osm',
                venue_type: 'outdoor',
            });
            ...
            try {
                ...
                if (result.status < 300) regionUpdated++;
            } catch (e) {
                // Skip
            }
        }
        ...
        totalWithRicherData += enrichable.length;
        if (regionUpdated > 0) console.log(`    → Updated ${regionUpdated} courts with richer data`);
    ...
    console.log(`  Courts with richer data: ${totalWithRicherData}`);
