/**
 * Import venues to production PostGIS database
 * Uses ST_MakePoint for geography column
 */
...
async function importPostGIS() {
    ...
    try {
        ...
        // Sample check
        const sample = await client.query(`SELECT name, city, access FROM courts WHERE access = 'members' LIMIT 3`);
        console.log('\n=== SAMPLE MEMBER COURTS ===');
