...
export class CourtsController {
  ...
  @Post("admin/migrate")
  async runMigrations(@Headers("x-user-id") userId: string) {
  ...
  async checkIn(
    ...
    try {
      ...
      this.notificationsService
        .sendCourtActivityNotification(courtId, courtName, userName, "check_in")
        .catch((err) =>
          console.error("Failed to send court activity notification:", err),
        );
    } catch (err) {
      console.error(
        "[CheckIn] Error fetching court/user for notification:",
        err,
      );
    }
